...
const UnitsForecast: React.FC = () => {
  ...
  const fetchForecast = async (productId: string) => {
    setLoading(true);
    try {
      let url = "http://127.0.0.1:8000/forecast/units";
      ...
      {forecastData && forecastData.prediction.length > 0 && (
        <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 max-w-sm">
          ...
          <p className="text-xl font-bold text-blue-700 mt-1">
            {forecastData.prediction[0].predictions.toFixed(0)}{" "}
            <span className="text-xs font-normal text-blue-500">units</span>
          </p>
        </div>
      )}
